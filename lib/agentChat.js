...
const API_AI_TOKEN = (keys.APIAITOKEN || process.env.APIAITOKEN);
//const API_AI_TOKEN = process.env.APIAITOKEN;//(keys.APIAITOKEN || process.env.APIAITOKEN);
...
class AgentChat {
            ...
            if (events) {
                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    if ((ev['@type'] === 'state') && (ev.state === 'ended')) {
                        ...
                        this.sendLine(ev.text, response);
                    }
                }
            }
    ...
    sendLine(userSaid, response) {
        const apiaiSession = apiAiClient.textRequest(userSaid, {sessionId: 'liveperson'});

        apiaiSession.on("response", (dfResponse) => {
            let speech = dfResponse.result.fulfillment.speech;
            console.log(`Sending line: "You said: ${userSaid}\nI'd say: ${speech}`);

            // Let's get the number of last processes message
            //console.log(JSON.stringify(response, null, 2));
            let ourHref = "";
            let eventID = "";
            if ("events" in response.body) {
                if ("link" in response.body.events) {
                    ourHref = response.body.events.link[1]["@href"];
                    eventID = ourHref.split("from=")[1];
                }
            }
            console.log("####################");
            ...
            const options = {
                ...
                body: {
                    event: {
                        '@type': 'line',
                        'text': `<p dir='ltr' style='direction: ltr; text-align: left;'>You said: ${userSaid} (event ID ${eventID})<br><br>I'd say: ${speech}</p>`,
                        'textType': 'html'
                    }
                }
            };
